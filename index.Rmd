...
subtitle: "European Doctoral School of Demography, 2025-2026 edition"
author: "Instructors: Diego Alburez-Gutierrez"
...
# Try to locate pandoc when rendering from terminal (outside RStudio).
if (!rmarkdown::pandoc_available()) {
  ...
  for (d in pandoc_dirs) {
    if (nzchar(d) && file.exists(file.path(d, "pandoc.exe"))) {
...
- Download the syllabus: https://github.com/alburezg/EDSD_2025_kinship/blob/master/EDSD_2025_26_kinship_syllabus.pdf 

- The course's slides are available here: https://github.com/alburezg/EDSD_2025_kinship/tree/master/slides

- Find this website's source code on GitHub: https://github.com/alburezg/EDSD_2025_kinship
...
<img src="img/DemoKin-Logo.png" align="right" width="200" />
...
It has years in columns and age in rows. Plotting $q_x$ ($p_x$Â´s complement) over age for 2018 gives:
...
In the lab sessions we will work with Brazil, so let's see the case of Brazil's female population in 2023.
For today's session the data is being provided; you will download it later as homework.

```{r 11, message=FALSE, warning=FALSE}
# Load the .RData file

load("data/brazil_data.RData") # px and fx - Brazil in 2023
```
...
# Download only if local files are missing
required_wpp_files <- c(
  "data/WPP2024_Fertility_by_Age1.csv",
  "data/WPP2024_Life_Table_Complete_Medium_Female_1950-2023.csv",
  "data/WPP2024_Life_Table_Complete_Medium_Male_1950-2023.csv",
  "data/WPP2024_Population1JanuaryBySingleAgeSex_Medium_1950-2023.csv"
)

if (all(file.exists(required_wpp_files))) {
  message("Using local WPP2024 CSV files from data/.")
} else {
  download_wpp24()
}
```
...
br_2023 <- kin(p = br_surv_2023, f = br_asfr_2023, time_invariant = TRUE, output_kin = "s")
```
...
##### 3.2.1 Using only the raw output in kin_full, get the mean age of living mother, daughter and sisters for a female aged 35.
...
What is the probability that Focal (an average woman in your country of choice) has a living mother over Focal's life? 
...
In today's session we will see how to apply the one-sex time-variant model and the two-sex time-variant and time-invariant models.
...
> `DemoKin` will only return values for either periods OR cohorts, but never for period-cohort combinations. This is related to time/memory issues. E.g., providing all possible period-cohort estimates in our example would give a data frame with 119 x 101 x 101 x 14 ~ 17 million rows.
...
The function `kin` provides information on the number of relatives lost by Focal during her life, stored in the column `kin_summary$count_cum_dead`. The plot below compares patterns of kin loss for the 1960 and 1990 cohorts.
...
- One-sex model; approximate kin of both sexes using GKP factors
...
Assignments (one per group) should be sent by email to alburezgutierrez@demogr.mpg.de **before midnight of Friday, March 6.** 
